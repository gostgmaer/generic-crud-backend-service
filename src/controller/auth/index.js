...
const reset = async (req, res) => {
    try {
        ...
        const request = await invoke(`/user/auth/reset-password/${token}`, param, req)
        res.status(request.statusCode).json(request);
    } catch (error) {
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            message: error.message,
...
const confirm = async (req, res) => {
    try {
        ...
        const { token } = req.params
        const request = await invoke(`/user/auth/confirm-account/${token}`, param, req)
        res.status(request.statusCode).json(request);
    } catch (error) {
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            message: error.message,
...
const refresh = async (req, res) => {
    try {
        ...
        const request = await invoke("/user/auth/session/refresh/token", param, req)
        res.status(request.statusCode).json(request);
    } catch (error) {
        res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({
            message: error.message,
...
module.exports = {
    register, login, signOut, refresh, verify, confirm, reset, forget, changes, profile
};
